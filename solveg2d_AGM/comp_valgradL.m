function [valL,gradrhoL,gradmxL,gradmyL] = comp_valgradL(siz,rho,mx,my,rho0,rho1,g,lambdaF,lambdaG)
...
msq = gxx.*mxc.^2 + 2*gxy.*mxc.*myc + gyy.*myc.^2;
...
% gradmxL(ind) = dt*(gxx(ind).*mxc(ind) + gxy(ind).*myc(ind))./rhoc(ind);
gradmxL = dt*(gxx.*mxc + gxy.*myc)./rhoc;
gradmxL(~ind) = 0;
gradmxL = (gradmxL(1:end-1,:,:)+gradmxL(2:end,:,:))/2;
% gradmyL = zeros(size(myc));
% gradmyL(ind) = dt*(gxy(ind).*mxc(ind) + gyy(ind).*myc(ind))./rhoc(ind);
gradmyL = dt*(gxy.*mxc + gyy.*myc)./rhoc;
gradmyL(~ind) = 0;

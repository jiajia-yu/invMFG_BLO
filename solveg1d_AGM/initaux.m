...
for n_data = 1:N
    ...
    mx_nit = m_cell{n_data};
    obj_array(n_data) = comp_valL(siz,rho_nit,mx_nit,rho0,rho1,g,lambdaF,lambdaG);
end
...
for nit = 1:maxit
    for n_data = 1:N
        ...
        % backtracking
        [gradyrho,gradymx] = comp_gradL(siz,rho_nit,mx_nit,rho0,rho1,g,lambdaF,lambdaG);
        subnit = 1;
        beta_nit = beta_init;
        while subnit < submaxit
            [rho_nitp,mx_nitp,projerr] = proj_rhom(rho_nit - beta_nit*gradyrho, ...
                mx_nit-beta_nit*gradymx,rho0);
            rho_nitp = max(rho_nitp,rhotol);

            obj_nitp = comp_valL(siz,rho_nitp,mx_nitp,rho0,rho1,g,lambdaF,lambdaG);
            diffrho = rho_nitp - rho_nit;
            diffmx = mx_nitp - mx_nit;
            G = obj_nit + dotsc(gradyrho,diffrho) + dotsc(gradymx,diffmx) + ...
                1/(2*beta_nit)*(dotsc(diffrho,diffrho) + dotsc(diffmx,diffmx));
        ...
        m_cell{n_data} = mx_nitp;
        m_cellhist{n_data,nit} = mx_nitp;

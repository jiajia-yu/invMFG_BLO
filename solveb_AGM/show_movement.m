function show_movement(rho,mx,my,obs,opts,filename)
...
if isfield(opts,'vec_dens') vec_dens = opts.vec_dens; else vec_dens = 4; end
if isfield(opts,'vec_leng') vec_leng = opts.vec_leng; else vec_leng = 0.5; end
...
for t = 1:ntp-1
%     show_example(squeeze(rho(:,:,t)),Qx);
    
%     imshow(max(squeeze(rho(:,:,t)),obs),[]);colormap default; colorbar
    imshow(squeeze(rho(:,:,t)),[]);colormap default; colorbar
    colormap default

    hold on

    quiver(X,Y,squeeze(my(1:vec_dens:end,1:vec_dens:end,t)),squeeze(mx(1:vec_dens:end,1:vec_dens:end,t)),...

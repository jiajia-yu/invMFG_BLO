function [valD_array,beta_array,rho_cellhist,m_cellhist] = initaux(mu_cell,w_cell,...
    rho0_cell,rho1_cell,b,lambdaF,lambdaG,rhotilde_cell,mtilde_cell,opts)
...
for n_data = 1:N
    ...
    obj_array(n_data) = comp_valL(siz,rho_nit,mx_nit,my_nit,rho0,rho1,b,lambdaF,lambdaG);
end
...
for nit = 1:maxit
    for n_data = 1:N
        ...
        % backtracking
        [gradyrho,gradymx,gradymy] = comp_gradL(siz,rho_nit,mx_nit,my_nit,rho0,rho1,b,lambdaF,lambdaG);
        subnit = 1;
        beta_nit = beta_init;
        while subnit < submaxit
            ...
            obj_nitp = comp_valL(siz,rho_nitp,mx_nitp,my_nitp,rho0,rho1,b,lambdaF,lambdaG);
